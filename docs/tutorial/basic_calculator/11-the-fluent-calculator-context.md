...
`calc_fluent.py` becomes one line: `calc_app = create_calculator_fluent()`. The `calc_fluent` session needs no context declaration in `config.yml` — `create_calculator_fluent` realizes `CalculatorFluentContext` itself, the same way `create_calculator_app` does in Chapter 9.
...
This is a shunting-yard algorithm for PEMDAS precedence, operating over the whole logged term list in one pass instead of interleaved with logging. `reduce` is typed as a `Callable`, handed in inline as an argument at the call site rather than stored or injected; `Expression` has no idea what actually computes `left op right`, only that whatever's passed in takes an operator and two operands and returns a number. `Expression.resolve` owns traversal and scheduling -- walking its own term list, deciding *when* each pairwise reduction happens -- while `reduce` decides *what* that reduction computes, kept deliberately one layer up so this domain object stays a pure scheduling algorithm with zero framework or DI knowledge. The old cache-backed `ExpressionContext` -- with its `load`/`save`/discard round-trip -- is retired entirely; there's nothing left to key by id and look up later, since the accumulating state is held directly rather than serialized out to a shared cache.
