...
`tiferet/domain/core.py` is the abstract core of the domain layer — the base every other domain object extends, the vocabulary for describing a model-level defect, and the shared shape for "a service implementation, named." It has zero framework imports, making the domain layer's "no framework dependencies" rule literally true. This guide gives `core.py` its own home (distinct from `docs/core/domain.md`'s code-style conventions) because it is depended on by every other domain module in this package and by the mappers, interfaces, and DI layers beyond it.
...
- **Model defect** — an inconsistency *within* a single model instance (unknown field, invalid value, or a mutation-policy refusal), as opposed to a domain outcome or an infrastructural failure.
...
Flattens Pydantic's `error.errors()` into `{'field', 'type', 'message'}` dicts, shared by both the mutation path (`Aggregate.set_attribute`) and the request-validation path (`contexts/feature.py::validate_request`).

### ModelError

A **standalone `Exception`, deliberately not a `TiferetError`**. A model inconsistency is a consumer defect, not a domain outcome — it is never catalogued, never resolved through the `Error` catalog, never formatted as a `TiferetAPIError`, and not skippable via a feature step's `pass_on_error`.

| Attribute | Type | Required | Default | Description |
|---|---|---|---|---|
| <a id="modelerror-error-code"></a>`error_code` | `str` | Yes | — | The model error code. |
| <a id="modelerror-model"></a>`model` | `Dict[str, Any]` | No | `{}` | The offending instance's descriptor, as produced by `describe_model`. |
| <a id="modelerror-violations"></a>`violations` | `List[Dict[str, Any]]` | No | `[]` | Structured field violations, when available. |
...
- **Mappers:** `Aggregate.set_attribute` (`docs/guides/mappers.md`) wraps `setattr` and converts any resulting `ValidationError` via `raise_for_validation`; whitelist overrides raise `ATTRIBUTE_NOT_SETTABLE` with the same descriptor.
- **Contexts:** `contexts/feature.py::validate_request` uses `unpack_validation_error` independently of the mutation path to flatten request-validation failures into `REQUEST_VALIDATION_FAILED` — a catalogued `TiferetError`, unlike `ModelError` itself.
- **App / DI:** `AppServiceDependency` (`docs/guides/domain/app.md`) and `ServiceRegistration`/`FlaggedDependency` (`docs/guides/domain/di.md`) both extend `ServiceDependency` rather than duplicating its three fields.
...
**Outside this domain:** mutation logic (owned by `Aggregate` subclasses in `mappers`), catalogued domain outcomes (`TiferetError`, see `docs/guides/errors.md`), and infrastructural failures (`ServiceError`, same guide). `ModelError` is deliberately excluded from all of those — it leaks as an unhandled exception by design.
