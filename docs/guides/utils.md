...
Extend `MiddlewareService` and implement `__call__`. Custom middleware is not restricted to `tiferet/utils/` — it may live in application code and be registered the same way. See [docs/guides/utils/core.md](utils/core.md#the-middlewareservice-pattern) for a worked example.
