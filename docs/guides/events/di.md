...
Creates a new `ServiceRegistration` and persists it via `DIService.save_registration()`. A configuration must define at least one type source: either a default type (`module_path` + `class_name`) or one or more flagged dependencies.
...
**Errors:**
- `SERVICE_REGISTRATION_ALREADY_EXISTS` if a configuration with the given ID already exists.
...
3. Creates a `ServiceRegistrationAggregate` via its `new()` factory.
4. Saves via `di_service.save_registration(configuration)`.
...
**Errors:**
- `SERVICE_REGISTRATION_NOT_FOUND` if the configuration does not exist.
...
**Returns:** `Tuple[List[ServiceRegistration], Dict[str, Any]]` — the list of configurations and the constants dictionary.
...
1. **Retrieve** the configuration via `di_service.get_registration(id)`.
...
4. **Save** the updated aggregate via `di_service.save_registration(configuration)`.
