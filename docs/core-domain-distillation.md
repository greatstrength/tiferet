...
see Section 8; a session declares no context class of its own on this path.
...
- **The entry-point axis is not actually driven by the declared session.**
  `build_app_session_context` builds a literal `AppSessionContext` directly,
  with no dynamic class resolution from the session's declaration. The CLI
  path instead exists
...
1. **Make the entry-point axis real, or leave it declared-but-inert.**
   Wiring `build_app_session_context` to actually resolve a context class
   from the session's declaration remains a design change for an RFP, not a
   defect fix. The vestigial `module_path` / `class_name` framing has been
   removed from the admin add surface, the example configuration, and the
   documentation that taught it (issue #1052).
...
4. **Audit other session-level configuration keys for the same silent-drop
   ...
   would surface a similar stale-key case early, before it accumulates.
