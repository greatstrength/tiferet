...
### Blueprint Logger Handler

This cycle relocates logger construction behind a fifth guarded template-method handler, retiring the hub's one remaining concrete sub-context dependency:

- **`AppSessionContext` gains a `build_logger` template method** (`contexts/app.py`), mirroring its four siblings: it raises `APP_ERROR` naming the missing `build_logger_handler` slot when unwired, and formats a `TiferetError` raised by the handler into a `TiferetAPIError` via `handle_error` so `run`'s pre-try region raises only `TiferetAPIError`. `run` calls `self.build_logger()` in place of the old `self.load_logging_context().build_logger()`.
- **`_logging`, `load_logging_context()`, and the `logging_context` constructor parameter are retired** from both `AppSessionContext` and `CliSessionContext`, along with the `from .logging import LoggingContext` import in `contexts/app.py` and `contexts/cli.py`. The hub now imports stdlib `logging` only, for the `build_logger` return-type annotation.
- **`blueprints/core.py::build_logging_context` is renamed and reduced to `create_logging_context(settings, logger_id) -> LoggingContext`**, a pure factory whose body is the single `LoggingContext.from_domain(...)` call; it takes no cache or resolver. The former inline merge logic is extracted as the pure `merge_logging_settings(cache, formatters, handlers, loggers) -> LoggingSettings` function (`# *** functions`), and a new `build_logger_handler(cache, get_dependency) -> Callable` blueprint (`# *** blueprints`) is structurally identical to `get_error` / `get_feature`: cache-first lookup under a new `LOGGER_CACHE_PREFIX = ('logging', 'loggers')` (`contexts/logging.py`), then `logging_list_all_evt` resolution, `merge_logging_settings`, `create_logging_context(...).build_logger()`, and a cache write-back. The cache-first branch means `logging.config.dictConfig` runs once per logger id per process rather than once per request.
- **`RESERVED_CONTEXT_PARAMETERS`** drops `'logging_context'` in favor of `'build_logger_handler'`.
- **`build_app_session_context` and `build_cli_session_context`** no longer call `build_logging_context` (nor pass `app_session.logger_id`) at compose time; they add `build_logger_handler=build_logger_handler(cache, resolver.get_dependency)` to their handler dicts instead. `logging_list_all_evt` is now consumed lazily, on the first `run()`, rather than eagerly during composition.
- **`blueprints/admin.py` and `blueprints/admin_cli.py` are unchanged** — out of this item's scope. Their `build_admin_app_session_context` / `build_admin_cli_session_context` functions do not wire `build_logger_handler` (they also never wired `logging_context`), so an admin session's first `run()` now raises a clean `APP_ERROR` naming the missing slot instead of the pre-existing unguarded `AttributeError` — a strict improvement, not a regression, and left for a future item to close.
- **`contexts/logging.py` is otherwise unchanged** — `LoggingContext`, `create_logger`, and `build_logger` keep their existing bodies; only the new `LOGGER_CACHE_PREFIX` constant is added.
- **Deferred** — Converting `LoggingContext.create_logger`'s two `TiferetError.raise_error` calls to a `ServiceError`; wiring `build_logger_handler` into the admin blueprints; per-request logger reconfiguration; the mirrored change on `main`.
