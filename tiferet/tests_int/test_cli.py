...
TEST_CALC_CONFIG = {
    ...
    'sessions': {
        ...
        'test_calc_cli': {
            ...
            'description': 'CLI session for testing calculator features via the CLI blueprint.',
            'constants': {
                'feature_config': None,
                'error_config': None,
                'di_config': None,
...
def test_calc_yaml_file(tmp_path, test_calc_cli_yaml_file):
    ...
    ``test_calc_cli`` session's ``cli_config`` constant.
    ...
    # Point the test_calc session constants at the main YAML file.
    for key in ('feature_config', 'error_config', 'di_config', 'logging_config'):
        config['sessions']['test_calc']['constants'][key] = str(file_path)

    # Point the test_calc_cli session constants at the main and CLI YAML files.
    for key in ('feature_config', 'error_config', 'di_config', 'logging_config'):
