...
def build_step_chain(
    ...
    # Build the async chain when the async path is requested.
    if is_async:
...
class FeatureContext(BaseContext):
    ...
    async def _execute_async(self, request: RequestContext, *flags, **kwargs):
        ...
        for event, step, params in self.resolve_feature_steps(request, *flags):
            ...
            # Await the step through the async execution chain.
    ...
    def resolve_feature_steps(self,
        ...
        tuples of ``(event, step, params)`` for each step that should execute.
