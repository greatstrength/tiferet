...
def test_init_without_initial_cache(cache_context):
    assert cache_context._cache == {}, "Cache should initialize empty if no data provided"
...
def test_delete_existing_key(cache_context):
    ...
    assert "to_delete" not in cache_context._cache, "Key should be deleted from cache"


# ** test: test_delete_non_existent_key
def test_delete_non_existent_key(cache_context):
    cache_context.delete("non_existent_key")
    assert "non_existent_key" not in cache_context._cache, "Deleting non-existent key should do nothing"
...
def test_clear(cache_context):
    ...
    cache_context.clear()
    assert cache_context._cache == {}, "Cache should be empty after clear"
