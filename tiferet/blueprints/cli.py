...
def build_argument_parser(commands: Dict[str, List[CliCommand]],
        parent_arguments: List[CliArgument]) -> argparse.ArgumentParser:
    '''
    Build an argparse parser from grouped CLI commands and parent arguments.

    Each argument is registered through ``CliArgument.to_argparse_kwargs()`` so
    the domain owns the translation from its declared input shape to the
    ...
    for group_key in commands:
        ...
        for cli_command in commands[group_key]:
            ...
            # Add the parent arguments that do not collide with command arguments.
...
def parse_cli_args_handler(list_commands_evt: Any,
    ...
    def handler(argv: Optional[List[str]] = None) -> Tuple[str, Dict[str, str], Dict[str, Any]]:
        ...
        active_command = next(
            (
                command for command in cli_commands
                if command.group_key == parsed.get('group', '')
                and command.key == parsed.get('command', '')
            ),
            None,
        )
        ...
        if active_command:
            for argument in active_command.arguments:
                dest = argument.get_dest()
                if dest in parsed:
                    parsed[dest] = argument.parse_value(parsed[dest])
...
def create_cli_request_context(interface_id: str,
        ...
        data: Dict[str, Any] = None) -> CliRequestContext:
    '''
    ...
    # Compose and return the CLI request context, stamping the interface id onto the headers.
...
def build_cli_session_context(app_session: AppSession, cache: CacheContext) -> CliSessionContext:
    ...
    :param cache: The pre-built shared cache context, seeded with the CLI
        commands in addition to the standard framework defaults.
...
def build_app(interface_id: str,
    ...
    cache seeded with the CLI commands, resolves the app session, composes the
    wired ``CliSessionContext``, and delegates argument parsing, feature
    dispatch, exit-code handling, and response formatting to
    ``CliSessionContext.run``.
    ...
    # Build the CLI cache, seeded with the errors, services, constants, and CLI commands.
